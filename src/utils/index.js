import viewsPath from "@/router/dictionary"
...
var array = []
export function getData(arr, type = 'children') {
  arr.forEach((res) => {
...
export function getViewPath(path) {
  ...
  const arr = viewsPath.filter(e => e.path === path)

  if (arr.length !== 0) {
    return arr[0]['view_path']
  }
