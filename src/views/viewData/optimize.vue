...
export default {
  ...
  methods: {
    changePlanTab(index, tab) {
      ...
      getCityColumnGraphApi(this.plan.tab, 4).then(res => {
    ...
    getList() {
      getOrderStatisticApi(4).then((res) => {
        res.data.forEach(i => {
      ...
      getYhPieApi().then(res => {
        this.stationPie = res.data
        this.drawRing1()
        this.drawRing3()
      })
      getUrgeOrderPageApi(this.page, this.size, 4).then(res => {
      ...
      getWorkOrderIssusesApi(4).then(res => {
      ...
      getOrderColumnGraphApi(4).then(res => {
